...
def calculateMaxPath(testArray,starting):
    ...
    valPath[startingi][startingj] = [sample[startingi][startingj]]
    for i in range(startingi,len(sample)-1):
       ...
       for j in range(0, len(valPath[i+1].keys())):
            if j == 0:
                valPath[i+1][j] = [valPath[i][j][0] + sample[i+1][j]]
            elif j >= len(valPath[i]):
                valPath[i+1][j] = [valPath[i][j-1][0] + sample[i+1][j]]
            else:
...
def main():
    ...
    a = readfromfilesamplenames("F:\\VirtualBOX\\DropBox\\Dropbox\\python\\Euler\\problem_067_data.txt")
##    print(a)
    testArray = convertToPyramid(a)
    b = calculateMaxPath(testArray,(0,0))
    listvals = []
    for e in b[len(b)-1].keys():
        listvals.append(b[len(b)-1][e])
